...
GitHub Actions cron (월 11:00 · 목 18:00 KST)
  ...
  → Slack 알림:  관리자 채널(자동 반영 안내) + 공지 채널(식단 업데이트)
```
...
### Slack 알림 (선택 — 없으면 알림만 건너뜀)

식단 반영 시 **관리자 채널 + 공지 채널에 각각 1건**씩 보냅니다.

| 시크릿 | 값 |
|---|---|
| `SLACK_BOT_TOKEN` | 슬랙 앱(봇) 토큰 `xoxb-...` — 봇에 `chat:write` 권한 필요 |
| `SLACK_CHANNEL_ADMIN` | 관리자 채널 ID — 자동 반영 안내 + 커밋 링크(원문 대조용) |
| `SLACK_CHANNEL_NOTICE` | 공지 채널 ID — 이번 주 식단 업데이트 안내(사이트 링크) |
| `SITE_URL` | (선택) 공지에 붙일 사이트 주소 |

- 봇을 **두 채널 모두에 초대**(`/invite @봇이름`)해야 전송됩니다.
- 채널 ID 는 슬랙에서 채널 이름 클릭 → 팝업 하단, 또는 채널 링크 끝부분.
- 한 채널만 쓰려면 그 채널 시크릿만 넣으면 됩니다(미설정 채널은 건너뜀). 두 시크릿에 **같은 채널 ID** 를 넣으면 그 채널에 2건이 나란히 옵니다.
